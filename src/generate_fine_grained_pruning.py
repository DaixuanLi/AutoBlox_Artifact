# experiment_name = "Fine_grained_layout_conf1_RC"
# perf_cat = "AdspayLoad"

# experiment_name = "Fine_grained_layout_conf1_DB"
# perf_cat = "TPCC"

# experiment_name = "Fine_grained_layout_conf1_WS"
# perf_cat = "WebSearch"

# conf_range = [340, 410]
# conf_range = [410, 500]

# experiment_name = "Fine_grained_layout_conf1_KV"
# perf_cat = "YCSB"

# experiment_name = "Fine_grained_layout_conf1_LM"
# perf_cat = "LiveMapsBackEnd"
# conf_range = [180, 360]
# conf_range = [200, 240]
# conf_range = [240, 280]
# conf_range = [280, 320]
...
# conf_range = [320, 400]
# conf_range = [400, 500]
# experiment_name += str(conf_range[0])


# experiment_name = "Fine_grained_layout_conf1_CS"
# perf_cat = "CloudStorage"
...
# experiment_name = "Fine_grained_layout_conf1_MR"
# perf_cat = "MapReduce"
# conf_range = [0, 330]
# conf_range = [280, 330]
# conf_range = [330, 380]
...
configuration_choices = {
    "Execution_Parameter_Set" : {
        ...
        "Device_Parameter_Set" : {
            ...
            "IO_Queue_Depth" : [4], # By increasing, Latency up but throughput up too,
            "Queue_Fetch_Size" : [4], # should do more survey on SSDs 
            "Caching_Mechanism" : ["ADVANCED"], # should add new cache strategy here 
            "Data_Cache_Sharing_Mode" : ["SHARED"], # 
            "Data_Cache_Capacity" : [838860800],
            "Data_Cache_DRAM_Row_Size" : [4096],
            "Data_Cache_DRAM_Data_Rate" : [2133], # for bus frequency of 100 MHz, DDR SDRAM 200GT/s
...
def check_conf_ok(conf):
    capacity = 1
    for i in range(0, len(capacity_control_id)):
        capacity *= capacity_control_vals[i][conf[capacity_control_id[i]]]
    if capacity < 0.85 * set_capacity or (capacity / 512) > (4 * 1024 * 1024 * 1024) or capacity > set_capacity:
        return False
    for i in range(0, len(size_ids[0])):
        if size_vals[0][i][conf[size_ids[0][i]]] > size_vals[1][i][conf[size_ids[1][i]]]:
            return False
    if calculate_punishment([base_conf], conf) > 4.0:
        return False
    return True
...
def get_next_conf(conf_prev):
    conf = conf_prev.copy()
    cur = len(working_set_ids) - 1
    curval = conf_prev[working_set_ids[cur]]
    while cur >= 0 and curval == conf_max[working_set_ids[cur]] - 1:
        conf[working_set_ids[cur]] = 0
        cur = cur - 1
        if cur >= 0:
            curval = conf_prev[working_set_ids[cur]]
    if cur == -1:
        ...
        conf[working_set_ids[cur]] = conf[working_set_ids[cur]] + 1
        return conf
...
while newconf[0] != -1:
    if not check_conf_ok(newconf):
        ...
        continue
    newconf = np.array(newconf).tolist()
    if newconf in confs:
        confid = confs.index(newconf)
    else:
        confs.append(newconf)
        # f_conf.write(json.dumps(np.array(newconf).tolist()))
        confid = len(confs) - 1
    print("generated valid confid:" + str(confid))
    # this is for debug usage
    # if confid < conf_range[0] or confid > conf_range[1]:
    #     newconf = get_next_conf(newconf)
    #     continue
    t_conf = conf2dict(newconf)
